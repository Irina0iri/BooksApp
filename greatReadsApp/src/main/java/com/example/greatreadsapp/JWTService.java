...
public class JWTService {
    ...
    public String generateTokenForUser(User user) {
        ...
        claims.put("email", user.get_email());

        return Jwts.builder().setSubject(user.get_email()).addClaims(claims)
                .setIssuedAt(new Date())
                .setExpiration(new Date(String.valueOf(new Date(System.currentTimeMillis() + 60*60*5*1000))))
